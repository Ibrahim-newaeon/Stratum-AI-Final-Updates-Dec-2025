...
async function mockApiResponses(page: Page) {
  ...
  await page.route('**/api/v1/tenants/*/emq/playbook', async (route) => {
    await route.fulfill({
      ...
      contentType: 'application/json',
      body: JSON.stringify({
        data: [{ id: '1', title: 'Fix Meta CAPI', priority: 'high', status: 'open' }],
      }),
    });
  });
  ...
  await page.route('**/api/v1/tenants/*/attribution-variance', async (route) => {
    await route.fulfill({
      ...
      contentType: 'application/json',
      body: JSON.stringify({
        data: [{ platform: 'meta', ga4_revenue: 10000, platform_revenue: 9500, variance_pct: 5 }],
      }),
    });
  });
...
test.describe('Signal Hub', () => {
  ...
  test('should render signal hub page', async ({ page }) => {
    ...
    await expect(page.locator('text=Signal Hub').or(page.locator('text=EMQ')).first()).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test('should display signal diagnostics', async ({ page }) => {
    ...
    const signalContent = page
      .locator('text=Signal Hub')
      .or(
        page
          .locator('text=EMQ')
          .or(page.locator('text=Recovery Metrics').or(page.locator('text=Active Incidents')))
      );
...
test.describe('Campaign Draft Flow', () => {
  ...
  test('should create campaign draft and see it in drafts list', async ({ page }) => {
    ...
    const nameInput = page
      ...
      .or(page.locator('input[placeholder*="campaign"]').or(page.locator('input').first()));
    ...
    const saveDraftButton = page
      ...
      .or(page.locator('button:has-text("Draft")'));
    ...
    const draftItem = page.locator('text=My New Campaign').or(page.locator('text=draft'));
  ...
  test('should display existing drafts', async ({ page }) => {
    ...
    const draftsContent = page
      .locator('text=Campaign Drafts')
      .or(
        page
          .locator('text=drafts')
          .or(page.locator('text=No Drafts Found').or(page.locator('text=Create New Draft')))
      );
...
test.describe('Publish Logs', () => {
  ...
  test('should display publish logs', async ({ page }) => {
    ...
    const logsContent = page
      .locator('text=Publish Logs')
      .or(
        page
          .locator('text=History of all campaign publish attempts')
          .or(page.locator('text=No publish logs found').or(page.locator('text=Campaign')))
      );
  ...
  test('should show publish log entry after publishing', async ({ page }) => {
    ...
    const logEntry = page
      ...
      .or(
        page
          .locator('text=Publish Logs')
          .or(page.locator('text=Summer Sale').or(page.locator('text=Campaign')))
      );
