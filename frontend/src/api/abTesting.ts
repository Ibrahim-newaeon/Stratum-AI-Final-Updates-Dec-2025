...
export const abTestingApi = {
  ...
  updateTest: async (testId: string, params: Partial<CreateTestParams>) => {
    const response = await apiClient.put<ApiResponse<ABTest>>(
      `/ab-testing/tests/${testId}`,
      params
    );
    return response.data.data;
  },
  ...
  stopTest: async (testId: string, winnerId?: string) => {
    const response = await apiClient.post<ApiResponse<ABTest>>(`/ab-testing/tests/${testId}/stop`, {
      winnerId,
    });
    return response.data.data;
  },
  ...
  getResults: async (testId: string) => {
    const response = await apiClient.get<ApiResponse<TestResults>>(
  ...
  getMetrics: async (testId: string, params?: { startDate?: string; endDate?: string }) => {
    const response = await apiClient.get<ApiResponse<TestMetrics[]>>(
      `/ab-testing/tests/${testId}/metrics`,
      { params }
    );
    return response.data.data;
  },
  ...
  getPowerAnalysis: async (params: {
    ...
    const response = await apiClient.post<ApiResponse<PowerAnalysis>>(
