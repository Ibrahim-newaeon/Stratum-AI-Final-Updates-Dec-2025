...
export interface SubscriptionStatus {
  ...
  /** ISO datetime string. Populated only during a free trial. */
  trial_ends_at?: string | null;
  /** True when the tenant is on a non-paid trial (e.g., 14-day Starter trial). */
  is_trial?: boolean;
}
...
export const subscriptionApi = {
  ...
  getWarnings: async (): Promise<SubscriptionWarningsResponse> => {
    const response =
      await apiClient.get<ApiResponse<SubscriptionWarningsResponse>>('/subscription/warnings');
    return response.data.data;
  },

  getUsageSummary: async (): Promise<SubscriptionUsageSummary> => {
    const response = await apiClient.get<ApiResponse<SubscriptionUsageSummary>>(
