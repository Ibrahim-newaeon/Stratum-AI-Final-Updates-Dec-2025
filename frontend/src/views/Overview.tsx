      ...
      <svg viewBox="0 0 120 40" className="w-full h-full" preserveAspectRatio="none">
        <defs>
          <linearGradient id={`spark-${positive}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={positive ? '#C9A227' : '#E85D5D'} stopOpacity="0.3" />
            <stop offset="100%" stopColor={positive ? '#C9A227' : '#E85D5D'} stopOpacity="0" />
          </linearGradient>
        </defs>
        <path
          d={positive
    ...
    <div className="surface-card p-5 hover:shadow-lg transition-shadow duration-200">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-md bg-[#C9A227]/10">
...
function PlatformHealthItem({
  ...
  const statusConfig = {
    ...
    syncing: { dot: 'bg-[#C9A227]', text: 'text-[#C9A227]', label: 'Syncing' },
...
function AIRecommendationCard({
  ...
  return (
    <div className="surface-card p-4 border-l-2 border-l-[#C9A227] hover:shadow-lg transition-shadow duration-200">
      <div className="flex items-start gap-3 mb-3">
        <div className="p-1.5 rounded-md bg-[#C9A227]/10 flex-shrink-0">
          <Brain className="h-4 w-4 text-[#C9A227]" />
        </div>
        <div>
      ...
      <div className="flex items-center justify-between mt-3">
        ...
        <button className="text-xs font-medium text-[#C9A227] hover:text-[#D4AF37] transition-colors duration-200 flex items-center gap-1">
    ...
    <div className="space-y-8">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setShowKeyboardHints(false)}>
          <div className="bg-[#0F1320] border border-[#1E2740] rounded-xl p-6 shadow-xl max-w-sm w-full mx-4 animate-in fade-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2 mb-4">
              <Keyboard className="w-5 h-5 text-[#C9A227]" />
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleSyncAll} disabled={syncAllMutation.isPending} className="inline-flex items-center px-4 py-2 border border-[#C9A227]/30 rounded-lg text-sm font-medium bg-[#C9A227]/5 text-[#C9A227] hover:bg-[#C9A227]/10 transition-colors duration-200 disabled:opacity-50" aria-label="Sync all campaigns from ad platforms" title="Pull latest data from Meta, TikTok, Snapchat & Google">
          ...
          <button onClick={handleExport} className="inline-flex items-center px-4 py-2 rounded-lg bg-[#C9A227] text-[#080C14] hover:bg-[#D4AF37] transition-colors duration-200 font-medium" aria-label="Export dashboard (E)">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {(['7D', '30D', '90D'] as const).map((range) => (
                <button
                  ...
                  className={cn(
                    ...
                    timeRange === range ? 'bg-[#C9A227]/10 text-[#C9A227]' : 'text-[#5A6278] hover:text-[#F0EDE5]'
                  )}
                >
                  {range}
                </button>
          ...
          <div className="h-64 w-full relative rounded-lg overflow-hidden bg-[#080C14] border border-[#1E2740]">
            <div className="absolute inset-0 opacity-30">
              <svg viewBox="0 0 600 200" className="w-full h-full" preserveAspectRatio="none">
                ...
                <path d="M0,180 C50,170 100,140 150,150 C200,120 250,80 300,90 C350,50 400,60 450,30 C500,20 550,10 600,5" fill="none" stroke="#C9A227" strokeWidth="2" />
        ...
        <div className="surface-card p-6">
          ...
          <button className="mt-4 w-full py-2 text-xs font-medium text-[#C9A227] hover:text-[#D4AF37] transition-colors duration-200 border border-[#1E2740] rounded-lg hover:border-[#C9A227]/30">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="surface-card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="text-xs text-[#C9A227] hover:text-[#D4AF37] transition-colors duration-200">View all</button>
          </div>
          {initialLoading ? (
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
