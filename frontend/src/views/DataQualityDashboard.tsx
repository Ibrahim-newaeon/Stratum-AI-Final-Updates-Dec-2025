...
export function DataQualityDashboard() {
    ...
    try {
      ...
      if (report && report.overall_score > 0) {
        ...
        if (report.platform_scores) {
          const platformData: PlatformEMQ[] = Object.entries(report.platform_scores).map(
        ...
        if (report.top_recommendations && report.top_recommendations.length > 0) {
          setRecommendations(
            report.top_recommendations.map((rec: any, idx: number) => ({
              id: String(idx + 1),
              impact:
                rec.severity === 'critical' ? 'high' : rec.severity === 'high' ? 'medium' : 'low',
  ...
  const filteredPlatformEMQ =
    platformFilter === 'all'
      ? platformEMQ
      : platformEMQ.filter((p) => p.platform === platformFilter);

  const filteredEventEMQ =
    eventFilter === 'all'
      ? eventEMQ
      : eventEMQ.filter((e) => {
  ...
  const filteredServerBrowserSplit =
    platformFilter === 'all'
      ? serverBrowserSplit
  ...
  const filteredAlerts = alerts.filter((alert) => {
    ...
    if (platformFilter !== 'all' && alert.platform && alert.platform !== platformFilter) {
      return false;
    }
    return true;
  });
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y">
                    {filteredPlatformEMQ.length > 0 ? (
                      filteredPlatformEMQ.map((row) => (
                        <tr key={row.platform} className="hover:bg-muted/30">
                          ...
                          <td
                            className={cn(
                              ...
                              getCoverageTextColor(row.overallEMQ)
                            )}
                          >
                            {row.overallEMQ}%
                          </td>
                          ...
                          <td className="text-right">
                            <span
                              className={cn(
                                ...
                                row.trend >= 0 ? 'text-green-500' : 'text-red-500'
                              )}
                            >
                              {row.trend >= 0 ? (
                                <TrendingUp className="w-3 h-3" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <tr key={row.parameter} className="hover:bg-muted/30">
                        <td className="py-2">
                          <span className="flex items-center gap-2">
                            {row.parameter}
                            {row.required && <span className="text-xs text-red-500">*</span>}
                          </span>
                        </td>
                          ...
                          <td key={idx} className="text-center py-2">
                            {value > 0 ? (
                              <div className="flex items-center justify-center">
                                <div
                                  className={cn(
                                    'w-10 h-6 rounded flex items-center justify-center text-xs font-medium text-white',
            ...
            <div className="rounded-xl border bg-card p-6">
              ...
              <div className="space-y-4">
                {filteredServerBrowserSplit.length > 0 ? (
                  filteredServerBrowserSplit.map((row) => (
            ...
            <div className="rounded-xl border bg-card p-6">
              ...
              <div className="space-y-3">
                {filteredIntegrityIssues.length > 0 ? (
                  filteredIntegrityIssues.map((issue, idx) => (
                    <div
                      key={idx}
                      className={cn(
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No pending data integrity issues
                    </p>
                  </div>
                )}
              </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-sm">
                              {violation.field}: {violation.violationType}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              Last seen: {violation.lastSeen}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
              ...
              {mockFunnelSteps.map((step, idx) => (
                <div key={step.name} className="flex items-center">
                  <div className="flex flex-col items-center min-w-[140px]">
                    ...
                    <span className={cn('text-2xl font-bold', getCoverageTextColor(step.emqScore))}>
                      {step.emqScore}%
                    </span>
                ...
                {(() => {
                  ...
                  return filteredRecommendations.length > 0 ? (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-muted-foreground">
                            Owner: <span className="font-medium text-foreground">{rec.owner}</span>
                          </span>
                        </div>
                    ...
                    <div className="flex flex-col items-center justify-center py-8 text-center">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Great job! All fixes have been implemented
                      </p>
                    </div>
                  );
                })()}
