      ...
      <div className="bg-white/[0.02] border border-white/10 rounded-xl p-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
          <div>
            ...
            <input type="date" value={filters.dateFrom} onChange={(e) => setFilters({ ...filters, dateFrom: e.target.value })} className="w-full bg-white/[0.03] border border-white/10 rounded px-2 py-1.5 text-sm text-white" />
          </div>
          <div>
            <label className="text-xs text-gray-400">To</label>
            <input type="date" value={filters.dateTo} onChange={(e) => setFilters({ ...filters, dateTo: e.target.value })} className="w-full bg-white/[0.03] border border-white/10 rounded px-2 py-1.5 text-sm text-white" />
          </div>
          <div>
            <label className="text-xs text-gray-400">Severity</label>
            <input value={filters.severity} onChange={(e) => setFilters({ ...filters, severity: e.target.value })} placeholder="info,warning,critical" className="w-full bg-white/[0.03] border border-white/10 rounded px-2 py-1.5 text-sm text-white placeholder-gray-600" />
          </div>
          <div>
            <label className="text-xs text-gray-400">Search</label>
            <div className="flex gap-1">
              <input value={filters.searchTerm} onChange={(e) => setFilters({ ...filters, searchTerm: e.target.value })} placeholder="keyword..." className="flex-1 bg-white/[0.03] border border-white/10 rounded px-2 py-1.5 text-sm text-white placeholder-gray-600" />
            </div>
          </div>
        </div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {displayRoles.map((role) => (
          <div key={role.id} className="bg-white/[0.02] border border-white/10 rounded-xl p-5">
        ...
        {preview && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={item.label} className="bg-white/[0.03] border border-white/5 rounded-lg p-4 text-center">
                <div className={`text-2xl font-bold ${item.color}`}>{item.count.toLocaleString()}</div>
                <div className="text-xs text-gray-500 mt-1">{item.label}</div>
              </div>
            ))}
            <div className="col-span-2 md:col-span-4 bg-white/[0.03] border border-white/5 rounded-lg p-4">
