...
export const getTenantId = (): number => {
  if (!currentTenantId) {
    ...
    currentTenantId = stored ? parseInt(stored, 10) : 12 // Default to 12 for now
  }
...
export const analyticsApi = {
  ...
  getDailyPerformance: async (params?: { days?: number }) => {
    const response = await apiClient.get('/analytics/daily-performance', { params })
    return response.data
  },
...
// CAPI / Data Quality API
export const capiApi = {
  // Quality endpoints
  getQualityReport: async (platforms?: string) => {
    const params = platforms ? { platforms } : {}
    const response = await apiClient.get('/capi/quality/report', { params })
    return response.data
  },

  getLiveInsights: async (platform: string = 'meta') => {
    const response = await apiClient.get('/capi/quality/live', { params: { platform } })
    return response.data
  },

  getIssues: async () => {
    const response = await apiClient.get('/capi/quality/issues')
    return response.data
  },

  resolveIssue: async (issueId: string) => {
    const response = await apiClient.post(`/capi/quality/issues/${issueId}/resolve`)
    return response.data
  },

  getAlerts: async (platform?: string) => {
    const params = platform ? { platform } : {}
    const response = await apiClient.get('/capi/quality/alerts', { params })
    return response.data
  },

  resolveAlert: async (alertId: string, action: 'auto_fix' | 'dismiss' = 'auto_fix') => {
    const response = await apiClient.post(`/capi/quality/alerts/${alertId}/resolve`, null, {
      params: { action },
    })
    return response.data
  },
  ...
  connectPlatform: async (platform: string, credentials: Record<string, string>) => {
  ...
  streamEvent: async (event: {
    ...
    const response = await apiClient.post('/capi/events/stream', event, {
      params: platforms ? { platforms: platforms.join(',') } : {},
    })
    return response.data
  },

  streamBatch: async (events: any[], platforms?: string[]) => {
    const response = await apiClient.post('/capi/events/batch', {
      events,
      platforms,
    })
    return response.data
  ...
  // EMQ by Platform
  getPlatformEMQ: async () => {
    const response = await apiClient.get('/capi/quality/platforms')
    return response.data
  },

  // EMQ by Event Type
  getEventQuality: async (platform?: string) => {
    const params = platform ? { platform } : {}
    const response = await apiClient.get('/capi/quality/events', { params })
    return response.data
  },

  // Parameter Coverage
  getParameterCoverage: async () => {
    const response = await apiClient.get('/capi/quality/parameters')
    return response.data
  },

  // Server vs Browser Split
  getDeliverySplit: async (platform?: string) => {
    const params = platform ? { platform } : {}
    const response = await apiClient.get('/capi/quality/delivery', { params })
    return response.data
  },

  // PII Compliance
  getPiiViolations: async () => {
    const response = await apiClient.get('/capi/quality/pii')
    return response.data
  },

  // Funnel Quality
  getFunnelQuality: async (platform?: string) => {
    const params = platform ? { platform } : {}
    const response = await apiClient.get('/capi/quality/funnel', { params })
    return response.data
  },
}
