...
function KpiItem({
  ...
  const change = kpi.previousValue !== undefined && typeof kpi.value === 'number'
    ? calculateChange(kpi.value, kpi.previousValue)
    : null

  const trend = kpi.trend || (change !== null
    ? change > 0 ? 'up' : change < 0 ? 'down' : 'flat'
    : undefined)

  const isTrendPositive = kpi.trendIsPositive !== false
    ? trend === 'up'
    : trend === 'down'
      ...
      {band && (
        <div className={cn(
          ...
          <span className={cn(
            ...
            {band === 'reliable' ? 'Reliable metrics' :
             band === 'directional' ? 'Directional metrics' :
