...
import { getStatusLabel, MetricCard, PlatformHealthRow, useSignalHealth } from '@/api/trustLayer';
...
const PlatformRow: React.FC<{ row: PlatformHealthRow }> = ({ row }) => {
  ...
  const getMetricStatus = (
    ...
    thresholds: { good: number; risk: number },
    inverse = false
  ) => {
      ...
      <td className="py-3 px-4">
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            {row.account_id && <div className="text-xs text-gray-500">{row.account_id}</div>}
          </div>
        </div>
      </td>
  ...
  if (!canSignalHealth) {
    return (
      <div className="bg-gray-50 rounded-lg border border-gray-200 p-6 text-center">
        <div className="text-gray-500">Signal Health feature is not enabled for your plan.</div>
      </div>
    );
  }
