...
export default function OnboardingChat({
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-[#D4AF37]/20 bg-gradient-to-r from-[#D4AF37]/10 to-[#14F0C6]/5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#D4AF37] flex items-center justify-center shadow-lg" style={{ boxShadow: '0 0 20px rgba(212, 175, 55, 0.4)' }}>
        ...
        <div className="h-1 bg-[#12121a]">
          <motion.div
            className="h-full bg-gradient-to-r from-[#D4AF37] to-[#14F0C6]"
            initial={{ width: 0 }}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              >
                    ...
                    <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-[#D4AF37]/10">
                      {message.quickReplies.map((reply, index) => (
                        <button
                          ...
                          disabled={isLoading}
                          className="px-3 py-1.5 text-xs font-medium bg-[#D4AF37]/10 hover:bg-[#D4AF37]/20 border border-[#D4AF37]/20 rounded-full text-[#D4AF37] hover:text-[#F4D03F] transition-colors disabled:opacity-50"
                        >
                          {reply}
                        </button>
                      ))}
                ...
                {message.requiresAction && message.actionType === 'oauth_redirect' && (
                  <button
                    ...
                    className="mt-3 w-full py-2 px-4 bg-[#D4AF37] hover:bg-[#F4D03F] text-[#0a0a0f] text-sm font-semibold rounded-lg transition-all hover:shadow-lg"
          ...
          {isLoading && (
            ...
            >
              <div className="bg-[#12121a] border border-[#D4AF37]/10 rounded-2xl px-4 py-3">
        ...
        {progress > 0 && progress < 100 && (
          <div className="px-4 py-2 border-t border-[#D4AF37]/10 bg-[#12121a]/50">
        ...
        {/* Completed State */}
        {progress === 100 && (
          <div className="px-4 py-3 border-t border-[#14F0C6]/20 bg-[#14F0C6]/10">
            <div className="flex items-center gap-2 text-[#14F0C6] text-sm font-medium">
        ...
        <div className="p-4 border-t border-[#D4AF37]/20 bg-[#0a0a0f]">
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 bg-[#12121a] border border-[#D4AF37]/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#D4AF37]/50 disabled:opacity-50 transition-colors"
            />
            <button
              onClick={() => sendMessage(inputValue)}
              disabled={!inputValue.trim() || isLoading || progress === 100}
              className="p-3 bg-[#D4AF37] hover:bg-[#F4D03F] rounded-xl text-[#0a0a0f] transition-all disabled:opacity-50 disabled:hover:bg-[#D4AF37]"
...
export function OnboardingChatButton({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        'fixed bottom-4 right-4 w-14 h-14 rounded-full bg-[#D4AF37] hover:bg-[#F4D03F] text-[#0a0a0f] shadow-lg hover:shadow-xl transition-all z-40 flex items-center justify-center',
        pulse && 'animate-pulse'
      )}
