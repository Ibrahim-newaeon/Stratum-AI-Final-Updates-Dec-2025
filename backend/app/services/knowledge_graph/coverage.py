...
BACKFILLED_EDGES = frozenset({"PERFORMED", "GENERATED", "BELONGS_TO", "RECEIVED"})

#: Node labels the batch backfill creates.
BACKFILLED_NODES = frozenset(
    {
        "Profile",
        "Campaign",
        "Segment",
...
_ATTRIBUTION_GRAIN = (
    "Requires per-conversion revenue attribution, which the schema stores only "
    "as period aggregates: daily_attributed_revenue is keyed (tenant, date, "
    "dimension) and conversion_paths by (path_hash, period), with no "
    "conversion id, while the graph's Revenue nodes are per event. No amount "
    "of loading produces the edge — closing this needs attribution persisted "
    "at conversion grain. Channel totals are served from the Channel node "
    "rollup instead; see sync_channels."
)
...
GAPS: dict[str, CoverageGap] = {
    ...
    "revenue_decline_root_cause": CoverageGap(
        ...
        reason=(
            f"{_ATTRIBUTION_GRAIN} Revenue decline is still detected; only the "
