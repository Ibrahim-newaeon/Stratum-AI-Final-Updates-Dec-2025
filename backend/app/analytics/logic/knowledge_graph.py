...
class KnowledgeCluster(BaseModel):
    ...
    performance_level: str = (
        "average"  # top / above_average / average / below_average / poor
    )
...
def build_knowledge_graph(campaigns: list[dict]) -> KnowledgeGraphResponse:
    ...
    for plat, data in platform_data.items():
        ...
        rel = (
            "drives"
            if plat_roas >= 3
            else "correlates_with" if plat_roas >= 1.5 else "inhibits"
        )
        edges.append(
            KnowledgeEdge(
                edge_id=f"e_{edge_idx}",
    ...
    high_roas = [
        ...
        if d["spend"] > 0 and d["revenue"] / d["spend"] >= 3
    ]
    low_roas = [
        (p, d)
        for p, d in platform_data.items()
        if d["spend"] > 0 and d["revenue"] / d["spend"] < 1.5
    ]
    ...
    high_vol_low_roas = [
        (p, d)
        for p, d in platform_data.items()
        if d["conversions"] > total_conversions * 0.3
        and d["spend"] > 0
        and d["revenue"] / d["spend"] < 2
    ]
    ...
    if overall_cpa > 0:
        patterns.append(
            KnowledgePattern(
                ...
                recommendation="Set CPA alerts at $"
                + f"{overall_cpa * 1.2:.2f} to catch cost increases early.",
                severity="info",
            )
        )
        pattern_idx += 1
    ...
    for level, (min_roas, max_roas) in [
        ...
        cluster_camps = [
            ...
            and min_roas
            <= float(c.get("revenue", 0)) / float(c.get("spend", 1))
            < max_roas
        ]
        if cluster_camps:
            ...
            c_plats = list(
                {
                    str(c.get("platform", "")).lower().replace("_", " ").title()
    ...
    strongest_desc = (
        f"{strongest.source} → {strongest.target} ({strongest.strength:.0f}%)"
        if strongest
        else "N/A"
    )
