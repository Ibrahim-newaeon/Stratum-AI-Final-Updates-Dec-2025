...
def build_journey_map(campaigns: list[dict]) -> JourneyMapResponse:
    ...
    platforms_sorted = sorted(
        platform_data.keys(), key=lambda p: platform_data[p]["spend"], reverse=True
    )
    ...
    for i, tmpl in enumerate(_JOURNEY_TEMPLATES):
        if tmpl["platforms"]:
            ...
            for tp in tmpl["platforms"]:
                ...
                plats.append(
                    match
                    or platforms_sorted[min(len(plats), len(platforms_sorted) - 1)]
                )
        else:
        ...
        total_freq += freq
        avg_rev = (
            total_revenue / total_conversions * tmpl["cvr"] / 3
    ...
    avg_touch = (
        sum(len(p.touchpoints) * p.frequency for p in top_paths) / total_freq
        if total_freq > 0
        else 0
    )
    avg_days = (
        sum(p.avg_days_to_convert * p.frequency for p in top_paths) / total_freq
        if total_freq > 0
        else 0
    )

    top_entry_ch = (
        max(contributions, key=lambda c: c.first_touch_pct).platform
        if contributions
        else "N/A"
    )
    top_close_ch = (
        max(contributions, key=lambda c: c.last_touch_pct).platform
        if contributions
        else "N/A"
    )
