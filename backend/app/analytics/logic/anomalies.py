    ...
    for metric in params.metrics_to_check:
        ...
        if is_anomaly or abs(z) >= 2.0:  # Include near-anomalies for visibility
            ...
            try:
                ...
                baseline_std = (
                    statistics.stdev(window_series) if len(window_series) > 1 else 0.0
                )
...
def detect_entity_anomalies(
    ...
    return {
        ...
        "has_critical": any(
            a.severity == AlertSeverity.CRITICAL for a in significant_anomalies
        ),
        "has_high": any(
            a.severity == AlertSeverity.HIGH for a in significant_anomalies
        ),
    }
...
def generate_anomaly_message(anomaly: AnomalyResult) -> str:
    ...
    pct_change = (
        abs(anomaly.current_value - anomaly.baseline_mean)
        / max(anomaly.baseline_mean, 1)
        * 100
    )
