...
class RecommendationsEngine:
    ...
    def generate_recommendations(
        ...
        return {
            ...
            "health": health_result.dict(),
            "scaling_summary": {
                "scale_candidates": len([s for s in scaling_results if s.action == ScalingAction.SCALE]),
                "fix_candidates": len([s for s in scaling_results if s.action == ScalingAction.FIX]),
                "watch_candidates": len([s for s in scaling_results if s.action == ScalingAction.WATCH]),
            },
            "generated_at": self.generated_at.isoformat(),
