...
def calculate_all_fatigue_scores():
    """
    Calculate fatigue scores for all creative assets using comprehensive analysis.
    Scheduled daily at 3 AM UTC.

    Uses AdFatigueService with 4-component scoring:
    - Usage Factor (0-25): Based on times_used across campaigns
    - Age Factor (0-25): Days since first deployment
    - Impression Saturation (0-25): Volume relative to audience
    - Performance Decay (0-25): CTR trend analysis
    """
    from app.services.ad_fatigue_service import AdFatigueService, FatigueStatus
    ...
    with SyncSessionLocal() as db:
        # Process by tenant for better organization
        tenants = db.execute(
            select(Tenant).where(Tenant.is_deleted == False)
        ).scalars().all()
        ...
        for tenant in tenants:
            ...
            for asset in assets:
                ...
                # Update asset with new score
                asset.fatigue_score = breakdown.total_score
                total_updated += 1

                # Track critical and fatigued assets
                if breakdown.status == FatigueStatus.CRITICAL:
                    critical_count += 1
                    tenant_critical += 1
                elif breakdown.status == FatigueStatus.FATIGUED:
                    tenant_fatigued += 1
            ...
            if assets:
                tenant_summaries.append({
                    ...
                    "fatigued": tenant_fatigued,
                })

                # Publish event for tenants with critical assets
                if tenant_critical > 0:
                    _publish_event(tenant.id, "fatigue_alert", {
                        "critical_assets": tenant_critical,
                        "fatigued_assets": tenant_fatigued,
                        "message": f"{tenant_critical} assets require immediate refresh",
                    })

        db.commit()

    logger.info(
        f"Updated fatigue scores for {total_updated} assets, {critical_count} critical"
    )
    return {
        "assets_updated": total_updated,
...
@shared_task(bind=True)
def calculate_tenant_fatigue_scores(self, tenant_id: int):
    """
    Calculate fatigue scores for a specific tenant's assets.
    Can be triggered on-demand from the UI.
    """
    from app.services.ad_fatigue_service import AdFatigueService, FatigueStatus

    logger.info(f"Calculating fatigue scores for tenant {tenant_id}")
    ...
    with SyncSessionLocal() as db:
        ...
        for asset in assets:
            ...
            if breakdown.status in [FatigueStatus.FATIGUED, FatigueStatus.CRITICAL]:
                summary["assets_needing_attention"].append({
                    ...
                    "recommendations": breakdown.recommendations[:2],
                })
        ...
        _publish_event(tenant_id, "fatigue_calculation_complete", {
            "total_assets": len(assets),
            "avg_score": summary["avg_fatigue_score"],
            "needs_attention": len(summary["assets_needing_attention"]),
        })

    logger.info(
        f"Tenant {tenant_id}: Updated {len(assets)} assets, "
        f"avg score {summary['avg_fatigue_score']}"
    )
