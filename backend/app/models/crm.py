...
class CRMConnection(Base):
    ...
    # Provider details
    provider = Column(SQLEnum(CRMProvider, name='crm_provider', native_enum=False), nullable=False)
    ...
    # Connection status
    status = Column(SQLEnum(CRMConnectionStatus, name='crm_connection_status', native_enum=False), nullable=False, default=CRMConnectionStatus.PENDING)
...
class CRMDeal(Base):
    ...
    stage_normalized = Column(SQLEnum(DealStage, name='deal_stage', native_enum=False), nullable=True)  # Normalized stage
    ...
    attribution_model = Column(SQLEnum(AttributionModel, name='attribution_model', native_enum=False), default=AttributionModel.LAST_TOUCH)
...
class CRMWritebackConfig(Base):
    ...
    # Last sync tracking
    last_sync_at = Column(DateTime(timezone=True), nullable=True)
    last_sync_status = Column(SQLEnum(WritebackStatus, name='writeback_status', native_enum=False), nullable=True)
...
class CRMWritebackSync(Base):
    ...
    status = Column(SQLEnum(WritebackStatus, name='writeback_status', native_enum=False), nullable=False, default=WritebackStatus.PENDING)
