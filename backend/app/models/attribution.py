...
class DailyAttributedRevenue(Base):
    ...
    attribution_model = Column(SQLEnum(AttributionModel, name='attribution_model', create_type=False), nullable=False)
...
class TrainedAttributionModel(Base):
    ...
    model_type = Column(SQLEnum(DataDrivenModelType, name='data_driven_model_type', create_type=False), nullable=False)
    channel_type = Column(String(50), nullable=False)  # platform, campaign

    # Status
    status = Column(SQLEnum(ModelStatus, name='model_status', create_type=False), nullable=False, default=ModelStatus.TRAINING)
