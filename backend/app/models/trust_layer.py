...
class FactSignalHealthDaily(Base):
    ...
    # Relationships
    tenant = relationship("Tenant", back_populates="signal_health_records")
...
class FactAttributionVarianceDaily(Base):
    ...
    # Relationships
    tenant = relationship("Tenant", back_populates="attribution_variance_records")
...
class FactActionsQueue(Base):
    ...
    applied_by_user_id = Column(Integer, ForeignKey("user.id", ondelete="SET NULL"), nullable=True)
    ...
    # Relationships
    tenant = relationship("Tenant", back_populates="actions_queue")
