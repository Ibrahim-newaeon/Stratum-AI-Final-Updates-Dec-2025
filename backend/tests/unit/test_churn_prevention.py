...
def _campaign(**kw):
    base = {
        "id": 1, "name": "C", "platform": "Meta", "status": "active",
        "spend": 100, "revenue": 300, "conversions": 30, "has_recent_sync": True,
    }
    base.update(kw)
    return base
...
class TestInterventions:
    def test_high_risk_creates_urgent_review_and_creative(self):
        signals = [
            ChurnSignal(signal="critical_roas", description="x",
                        severity="critical", weight=0.35),
        ]
        interv = cp._generate_interventions(
            risk_score=75, signals=signals, roas=0.3, conversions=0,
            spend=200, platform="Meta",
        )
    ...
    def test_conversion_signal_is_auto_eligible(self):
        signals = [
            ChurnSignal(signal="zero_conversions", description="x",
                        severity="critical", weight=0.2),
        ]
        interv = cp._generate_interventions(
            risk_score=30, signals=signals, roas=1.5, conversions=0,
            spend=200, platform="Meta",
        )
    ...
    def test_stale_data_triggers_resync_with_platform_name(self):
        signals = [
            ChurnSignal(signal="stale_data", description="x",
                        severity="medium", weight=0.1),
        ]
        interv = cp._generate_interventions(
            risk_score=30, signals=signals, roas=2.0, conversions=10,
            spend=200, platform="TikTok",
        )
    ...
    def test_mid_risk_adds_proactive_outreach(self):
        ...
        signals = [
            ChurnSignal(signal="spend_declining", description="x",
                        severity="high", weight=0.15),
        ]
        interv = cp._generate_interventions(
            risk_score=45, signals=signals, roas=2.0, conversions=10,
            spend=100, platform="Meta",
        )
...
class TestBuildChurnPrevention:
    ...
    def test_critical_and_healthy_mix(self):
        campaigns = [
            _campaign(id=1, name="Failing", spend=200, revenue=20, conversions=0,
                      status="paused", has_recent_sync=False),
            _campaign(id=2, name="Winner", spend=1000, revenue=5000,
                      conversions=100, status="active", has_recent_sync=True),
        ]
    ...
    def test_risk_campaigns_sorted_descending(self):
        campaigns = [
            ...
            _campaign(id=2, name="Severe", spend=300, revenue=15, conversions=0,
                      status="paused", has_recent_sync=False),
        ]
        resp = build_churn_prevention(campaigns)
...
class TestSummary:
    def test_summary_assembles_all_parts(self):
        interv = [
            Intervention(action="conversion_audit", title="t", description="d",
                         priority="immediate", category="technical"),
        ]
        s = cp._build_summary(
            total=5, at_risk=2, critical=1, healthy=3,
            retention_rate=60.0, portfolio_level="warning",
    ...
    def test_summary_all_healthy(self):
        s = cp._build_summary(
            total=3, at_risk=0, critical=0, healthy=3,
            retention_rate=100.0, portfolio_level="healthy",
            top_interventions=[],
        )
