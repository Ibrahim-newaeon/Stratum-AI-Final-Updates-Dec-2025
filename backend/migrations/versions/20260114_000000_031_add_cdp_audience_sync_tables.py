...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "sync_interval_hours", sa.Integer(), server_default="24", nullable=False
        ),
        ...
        sa.Column(
            "platform_config", postgresql.JSONB(), server_default="{}", nullable=False
        ),
        # Timestamps
        sa.Column(
            "created_at",
        ...
        sa.ForeignKeyConstraint(
            ["segment_id"], ["cdp_segments.id"], ondelete="CASCADE"
        ),
        sa.UniqueConstraint(
            "tenant_id",
            "segment_id",
    ...
    op.create_index(
        "ix_platform_audiences_segment", "platform_audiences", ["segment_id"]
    )
    op.create_index(
        "ix_platform_audiences_platform",
        "platform_audiences",
        ["tenant_id", "platform"],
    )
    op.create_index(
        "ix_platform_audiences_account",
        "platform_audiences",
    ...
    op.create_table(
        ...
        sa.Column(
            "platform_audience_id", postgresql.UUID(as_uuid=True), nullable=False
        ),
        # Job details
        ...
        sa.Column(
            "error_details", postgresql.JSONB(), server_default="{}", nullable=False
        ),
        # Platform response
        sa.Column(
            "platform_response", postgresql.JSONB(), server_default="{}", nullable=False
        ),
        # Triggered by
        ...
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["platform_audience_id"], ["platform_audiences.id"], ondelete="CASCADE"
        ),
    )
    ...
    op.create_index(
        "ix_audience_sync_jobs_audience", "audience_sync_jobs", ["platform_audience_id"]
    )
    op.create_index(
        "ix_audience_sync_jobs_status", "audience_sync_jobs", ["tenant_id", "status"]
    )
    op.create_index(
        "ix_audience_sync_jobs_created", "audience_sync_jobs", ["created_at"]
    )
    ...
    op.create_index(
        "ix_audience_sync_creds_tenant", "audience_sync_credentials", ["tenant_id"]
    )
    op.create_index(
        "ix_audience_sync_creds_platform",
        "audience_sync_credentials",
        ["tenant_id", "platform"],
    )
