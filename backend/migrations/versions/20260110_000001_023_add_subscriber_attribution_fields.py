...
def upgrade() -> None:
    ...
    # Phone number
    op.add_column(
        "landing_page_subscribers", sa.Column("phone", sa.String(20), nullable=True)
    )

    # Additional UTM parameters
    op.add_column(
        "landing_page_subscribers", sa.Column("utm_term", sa.String(100), nullable=True)
    )
    op.add_column(
        "landing_page_subscribers",
    ...
    op.add_column(
        "landing_page_subscribers", sa.Column("capi_results", sa.Text(), nullable=True)
    )
    ...
    # Create new indexes
    op.create_index(
        "ix_subscriber_platform", "landing_page_subscribers", ["attributed_platform"]
    )
    op.create_index(
        "ix_subscriber_lead_score", "landing_page_subscribers", ["lead_score"]
    )
