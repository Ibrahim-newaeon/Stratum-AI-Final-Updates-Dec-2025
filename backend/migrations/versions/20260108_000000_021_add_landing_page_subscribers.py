...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # Source tracking
        sa.Column(
            "source_page", sa.String(50), nullable=False, server_default="landing"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["converted_to_tenant_id"], ["tenants.id"], ondelete="SET NULL"
        ),
    )
    ...
    op.create_index(
        "ix_subscriber_status", "landing_page_subscribers", ["status", "created_at"]
    )
    op.create_index(
        "ix_subscriber_source", "landing_page_subscribers", ["source_page", "language"]
    )
