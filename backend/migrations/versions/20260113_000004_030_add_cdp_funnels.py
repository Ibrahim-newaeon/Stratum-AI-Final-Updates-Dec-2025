...
def upgrade() -> None:
    ...
    op.create_unique_constraint(
        "uq_cdp_funnels_slug", "cdp_funnels", ["tenant_id", "slug"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "funnel_id",
            UUID(as_uuid=True),
            sa.ForeignKey("cdp_funnels.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "profile_id",
            UUID(as_uuid=True),
            sa.ForeignKey("cdp_profiles.id", ondelete="CASCADE"),
            nullable=False,
        ),
        # Entry status
        sa.Column(
    ...
    op.create_index(
        "ix_cdp_funnel_entries_profile", "cdp_funnel_entries", ["profile_id"]
    )
    op.create_index(
        "ix_cdp_funnel_entries_converted",
        "cdp_funnel_entries",
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "uq_cdp_funnel_entries_funnel_profile", "cdp_funnel_entries", type_="unique"
    )
