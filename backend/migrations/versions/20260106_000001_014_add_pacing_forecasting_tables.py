...
revision = "014_pacing"
down_revision = "013_crm_integration"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "crm_opportunities", sa.Integer(), server_default="0", nullable=False
        ),
        sa.Column("crm_deals_won", sa.Integer(), server_default="0", nullable=False),
        sa.Column(
            "crm_pipeline_cents", sa.BigInteger(), server_default="0", nullable=False
        ),
        sa.Column(
            "crm_won_revenue_cents", sa.BigInteger(), server_default="0", nullable=False
        ),
        # Computed CRM ROAS
        ...
        # Constraints
        sa.UniqueConstraint(
            "tenant_id", "date", "platform", "campaign_id", name="uq_daily_kpi_scope"
        ),
    )

    op.create_index("ix_daily_kpis_tenant_date", "daily_kpis", ["tenant_id", "date"])
    op.create_index(
        "ix_daily_kpis_tenant_platform_date",
        "daily_kpis",
        ["tenant_id", "platform", "date"],
    )
    op.create_index(
        "ix_daily_kpis_tenant_campaign_date",
        "daily_kpis",
    ...
    op.create_index(
        "ix_pacing_alerts_tenant_status", "pacing_alerts", ["tenant_id", "status"]
    )
    op.create_index(
        "ix_pacing_alerts_tenant_date", "pacing_alerts", ["tenant_id", "pacing_date"]
    )
    op.create_index(
        "ix_pacing_alerts_tenant_type", "pacing_alerts", ["tenant_id", "alert_type"]
    )
    ...
    op.create_index(
        "ix_forecasts_tenant_date", "forecasts", ["tenant_id", "forecast_date"]
    )
    op.create_index(
        "ix_forecasts_tenant_for_date", "forecasts", ["tenant_id", "forecast_for_date"]
    )
    op.create_index(
        "ix_forecasts_tenant_metric", "forecasts", ["tenant_id", "metric_type"]
    )
    ...
    op.create_index(
        "ix_pacing_summaries_tenant_date",
        "pacing_summaries",
        ["tenant_id", "snapshot_date"],
    )
    op.create_index(
        "ix_pacing_summaries_target", "pacing_summaries", ["target_id", "snapshot_date"]
    )
