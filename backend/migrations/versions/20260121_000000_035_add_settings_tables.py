...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("active", "paused", "failed", name="webhook_status"),
    ...
    op.create_table(
        ...
        sa.Column(
            "type",
            sa.Enum(
                "info", "warning", "error", "success", "alert", name="notification_type"
            ),
            server_default="info",
            nullable=False,
        ),
    ...
    op.create_index(
        "ix_changelog_published", "changelog_entries", ["is_published", "published_at"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["changelog_id"], ["changelog_entries.id"], ondelete="CASCADE"
        ),
    )

    op.create_index(
        "ix_changelog_read_user",
        "changelog_read_status",
    ...
    op.create_table(
        ...
        sa.Column(
            "notify_trust_gate", sa.Boolean(), server_default="true", nullable=False
        ),
        sa.Column(
            "notify_anomalies", sa.Boolean(), server_default="true", nullable=False
        ),
        sa.Column(
            "notify_signal_health", sa.Boolean(), server_default="false", nullable=False
        ),
        sa.Column(
            "notify_daily_summary", sa.Boolean(), server_default="true", nullable=False
        ),
        # Status
    ...
    op.create_index(
        "ix_slack_integrations_tenant", "slack_integrations", ["tenant_id"], unique=True
    )
