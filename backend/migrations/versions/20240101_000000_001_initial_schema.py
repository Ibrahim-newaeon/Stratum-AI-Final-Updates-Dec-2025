...
def upgrade() -> None:
    # Create enum types
    op.execute(
        "CREATE TYPE userrole AS ENUM ('superadmin', 'admin', 'manager', 'analyst', 'viewer')"
    )
    op.execute(
        "CREATE TYPE adplatform AS ENUM ('meta', 'google', 'tiktok', 'snapchat')"
    )
    op.execute(
        "CREATE TYPE campaignstatus AS ENUM ('draft', 'active', 'paused', 'completed', 'archived')"
    )
    op.execute(
        "CREATE TYPE assettype AS ENUM ('image', 'video', 'carousel', 'story', 'html5')"
    )
    op.execute("CREATE TYPE rulestatus AS ENUM ('active', 'paused', 'draft')")
    op.execute(
        "CREATE TYPE ruleoperator AS ENUM ('equals', 'not_equals', 'greater_than', 'less_than', 'gte', 'lte', 'contains', 'in')"
    )
    op.execute(
        "CREATE TYPE ruleaction AS ENUM ('apply_label', 'send_alert', 'pause_campaign', 'adjust_budget', 'notify_slack')"
    )
    op.execute(
        "CREATE TYPE auditaction AS ENUM ('create', 'update', 'delete', 'login', 'logout', 'export', 'anonymize')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "consent_marketing", sa.Boolean(), nullable=False, server_default="false"
        ),
        sa.Column(
            "consent_analytics", sa.Boolean(), nullable=False, server_default="true"
        ),
    ...
    op.create_index(
        "ix_users_tenant_active", "users", ["tenant_id", "is_active", "is_deleted"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "total_spend_cents", sa.Integer(), nullable=False, server_default="0"
        ),
        sa.Column(
        ...
        sa.Column(
            "targeting_genders", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "targeting_locations",
            postgresql.JSONB(astext_type=sa.Text()),
        ...
        sa.Column(
            "demographics_age", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "demographics_gender",
            postgresql.JSONB(astext_type=sa.Text()),
    ...
    op.create_index(
        "ix_campaigns_date_range", "campaigns", ["tenant_id", "start_date", "end_date"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "demographics", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.ForeignKeyConstraint(
            ["campaign_id"],
            ["campaigns.id"],
            name="fk_campaign_metrics_campaign_id_campaigns",
            ondelete="CASCADE",
        ),
    ...
    op.create_index(
        "ix_campaign_metrics_date", "campaign_metrics", ["tenant_id", "date"]
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["campaign_id"],
            ["campaigns.id"],
            name="fk_creative_assets_campaign_id_campaigns",
            ondelete="SET NULL",
        ),
    ...
    op.create_index(
        "ix_assets_tenant_type", "creative_assets", ["tenant_id", "asset_type"]
    )
    op.create_index(
        "ix_assets_fatigue", "creative_assets", ["tenant_id", "fatigue_score"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            postgresql.ENUM(
                "active", "paused", "draft", name="rulestatus", create_type=False
            ),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "condition_result", postgresql.JSONB(astext_type=sa.Text()), nullable=False
        ),
        sa.Column(
            "action_result", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column("error", sa.Text(), nullable=True),
        sa.ForeignKeyConstraint(
            ["campaign_id"],
            ["campaigns.id"],
            name="fk_rule_executions_campaign_id_campaigns",
            ondelete="SET NULL",
        ),
        sa.ForeignKeyConstraint(
            ["rule_id"],
            ["rules.id"],
            name="fk_rule_executions_rule_id_rules",
            ondelete="CASCADE",
        ),
    ...
    op.create_index(
        "ix_rule_executions_rule_date", "rule_executions", ["rule_id", "executed_at"]
    )
    op.create_index(
        "ix_rule_executions_tenant_date",
        "rule_executions",
    ...
    op.create_table(
        ...
        sa.Column(
            "meta_keywords", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
        ...
        sa.Column(
            "top_keywords", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        ...
        sa.Column(
            "metrics_history", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "data_source",
            sa.String(length=50),
    ...
    op.create_index(
        "ix_competitor_benchmarks_tenant_id", "competitor_benchmarks", ["tenant_id"]
    )
    op.create_index(
        "ix_competitors_tenant_primary",
        "competitor_benchmarks",
        ["tenant_id", "is_primary"],
    )
    op.create_index(
        "ix_competitors_sov", "competitor_benchmarks", ["tenant_id", "share_of_voice"]
    )
    ...
    op.create_index(
        "ix_audit_action", "audit_logs", ["tenant_id", "action", "created_at"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["campaign_id"],
            ["campaigns.id"],
            name="fk_ml_predictions_campaign_id_campaigns",
            ondelete="CASCADE",
        ),
    ...
    op.create_index(
        "ix_predictions_cache", "ml_predictions", ["model_type", "input_hash"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "alert_rule_triggered", sa.Boolean(), nullable=False, server_default="true"
        ),
        sa.Column(
        ...
        sa.Column(
            "alert_sync_failure", sa.Boolean(), nullable=False, server_default="false"
        ),
        ...
        sa.Column(
            "report_monthly", sa.Boolean(), nullable=False, server_default="true"
        ),
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
