...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "platform_response", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
    ...
    op.create_index(
        "ix_capi_delivery_tenant_time",
        "capi_delivery_logs",
        ["tenant_id", "delivery_time"],
    )
    op.create_index(
        "ix_capi_delivery_platform",
        "capi_delivery_logs",
        ["tenant_id", "platform", "delivery_time"],
    )
    op.create_index(
        "ix_capi_delivery_status",
        "capi_delivery_logs",
        ["tenant_id", "status", "delivery_time"],
    )
    op.create_index(
        "ix_capi_delivery_event_id",
        "capi_delivery_logs",
        ["tenant_id", "platform", "event_id"],
    )
    ...
    op.create_index(
        "ix_dlq_pending",
        "capi_dead_letter_queue",
        ["tenant_id", "status", "last_failure_at"],
    )
    op.create_index(
        "ix_dlq_platform", "capi_dead_letter_queue", ["tenant_id", "platform", "status"]
    )
    op.create_index(
        "ix_dlq_category",
        "capi_dead_letter_queue",
        ["tenant_id", "failure_category", "first_failure_at"],
    )
    ...
    op.create_index(
        "ix_dedupe_key", "capi_event_dedupe", ["tenant_id", "dedupe_key"], unique=True
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "successful_events", sa.Integer(), nullable=False, server_default="0"
        ),
        ...
        sa.Column(
            "deduplicated_events", sa.Integer(), nullable=False, server_default="0"
        ),
        ...
        sa.Column(
            "total_value_cents", sa.BigInteger(), nullable=False, server_default="0"
        ),
        sa.Column("avg_value_cents", sa.Integer(), nullable=True),
        sa.Column(
            "error_counts", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
    ...
    op.create_index(
        "ix_delivery_stats_date",
        "capi_delivery_daily_stats",
        ["tenant_id", "date", "platform"],
    )
    op.create_index(
        "ix_delivery_stats_unique",
        "capi_delivery_daily_stats",
        ["tenant_id", "date", "platform"],
        unique=True,
    )
