...
def upgrade() -> None:
    ...
    op.add_column(
        "cms_posts", sa.Column("submitted_by_id", sa.Integer(), nullable=True)
    )

    # Review tracking
    op.add_column(
        "cms_posts", sa.Column("reviewed_at", sa.DateTime(timezone=True), nullable=True)
    )
    ...
    # Approval tracking
    op.add_column(
        "cms_posts", sa.Column("approved_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column("cms_posts", sa.Column("approved_by_id", sa.Integer(), nullable=True))

    # Rejection tracking
    op.add_column(
        "cms_posts", sa.Column("rejected_at", sa.DateTime(timezone=True), nullable=True)
    )
    ...
    op.add_column(
        "cms_posts", sa.Column("current_version_id", sa.UUID(), nullable=True)
    )

    # Assigned reviewer
    op.add_column(
        "cms_posts", sa.Column("assigned_reviewer_id", sa.Integer(), nullable=True)
    )
    op.add_column(
        "cms_posts",
    ...
    op.add_column(
        "cms_posts", sa.Column("locked_at", sa.DateTime(timezone=True), nullable=True)
    )
    ...
    op.create_index(
        "ix_cms_post_versions_version", "cms_post_versions", ["post_id", "version"]
    )
    ...
    op.create_index(
        "ix_cms_workflow_logs_user", "cms_workflow_logs", ["performed_by_id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_cms_posts_assigned_reviewer", "cms_posts", type_="foreignkey"
    )
