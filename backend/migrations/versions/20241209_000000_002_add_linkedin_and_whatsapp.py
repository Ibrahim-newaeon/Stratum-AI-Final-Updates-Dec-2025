...
def upgrade() -> None:
    ...
    # =========================================================================
    op.execute(
        "CREATE TYPE whatsappoptinstatus AS ENUM ('pending', 'opted_in', 'opted_out')"
    )
    ...
    op.execute(
        "CREATE TYPE whatsapptemplatestatus AS ENUM ('pending', 'approved', 'rejected', 'paused')"
    )
    op.execute(
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            nullable=False,
            server_default='["alerts", "reports", "digests"]',
        ),
        sa.Column(
            "quiet_hours",
            postgresql.JSONB(astext_type=sa.Text()),
            nullable=False,
            server_default='{"enabled": false, "start": "22:00", "end": "08:00"}',
        ),
        sa.Column(
        ...
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
            name="fk_whatsapp_contacts_user_id_users",
            ondelete="SET NULL",
        ),
        sa.PrimaryKeyConstraint("id", name="pk_whatsapp_contacts"),
    )
    op.create_index(
        "ix_wa_contacts_tenant_phone",
        "whatsapp_contacts",
        ["tenant_id", "phone_number"],
    )
    op.create_index("ix_wa_contacts_user", "whatsapp_contacts", ["user_id"])
    op.create_index(
        "ix_wa_contacts_opt_in", "whatsapp_contacts", ["tenant_id", "opt_in_status"]
    )
    ...
    op.create_index(
        "ix_wa_templates_tenant_status", "whatsapp_templates", ["tenant_id", "status"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            name="fk_whatsapp_messages_contact_id",
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["template_id"],
            ["whatsapp_templates.id"],
            name="fk_whatsapp_messages_template_id",
            ondelete="SET NULL",
        ),
        sa.PrimaryKeyConstraint("id", name="pk_whatsapp_messages"),
    )
    op.create_index(
        "ix_wa_messages_contact", "whatsapp_messages", ["contact_id", "created_at"]
    )
    op.create_index(
        "ix_wa_messages_status", "whatsapp_messages", ["tenant_id", "status"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["contact_id"],
            ["whatsapp_contacts.id"],
            name="fk_whatsapp_conversations_contact_id",
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name="pk_whatsapp_conversations"),
    )
    op.create_index(
        "ix_wa_conversations_contact", "whatsapp_conversations", ["contact_id"]
    )
    op.create_index(
        "ix_wa_conversations_active",
        "whatsapp_conversations",
