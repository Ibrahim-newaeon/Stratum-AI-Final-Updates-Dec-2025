...
revision = "012_add_autopilot_enforcement"
...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE enforcement_mode AS ENUM ('advisory', 'soft_block', 'hard_block')"
    )
    op.execute(
        "CREATE TYPE violation_type AS ENUM ('budget_exceeded', 'roas_below_threshold', 'daily_spend_limit', 'campaign_pause_required', 'frequency_cap_exceeded')"
    )
    op.execute(
        "CREATE TYPE intervention_action AS ENUM ('warned', 'blocked', 'auto_paused', 'override_logged', 'notification_sent', 'kill_switch_changed')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "enforcement_enabled", sa.Boolean(), nullable=False, server_default="true"
        ),
        sa.Column(
            "default_mode",
            enforcement_mode_enum,
            nullable=False,
        ...
        sa.Column(
            "min_roas_threshold", sa.Float(), nullable=False, server_default="1.0"
        ),
        sa.Column(
            "roas_lookback_days", sa.Integer(), nullable=False, server_default="7"
        ),
        sa.Column(
    ...
    op.create_index(
        "ix_tenant_enforcement_settings_tenant_id",
        "tenant_enforcement_settings",
        ["tenant_id"],
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["settings_id"], ["tenant_enforcement_settings.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_index(
        "ix_tenant_enforcement_rules_tenant_id",
        "tenant_enforcement_rules",
        ["tenant_id"],
    )
    op.create_index(
        "ix_tenant_enforcement_rules_settings_id",
        "tenant_enforcement_rules",
        ["settings_id"],
    )
    ...
    op.create_index(
        "ix_enforcement_audit_logs_tenant_id", "enforcement_audit_logs", ["tenant_id"]
    )
    op.create_index(
        "ix_enforcement_audit_logs_timestamp", "enforcement_audit_logs", ["timestamp"]
    )
    op.create_index(
        "ix_enforcement_audit_logs_action_type",
        "enforcement_audit_logs",
        ["action_type"],
    )
    ...
    op.create_index(
        "ix_pending_confirmation_tokens_token", "pending_confirmation_tokens", ["token"]
    )
    op.create_index(
        "ix_pending_confirmation_tokens_tenant_id",
        "pending_confirmation_tokens",
        ["tenant_id"],
    )
    op.create_index(
        "ix_pending_confirmation_tokens_expires_at",
        "pending_confirmation_tokens",
        ["expires_at"],
    )
