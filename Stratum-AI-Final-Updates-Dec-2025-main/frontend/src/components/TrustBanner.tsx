...
import { getStatusLabel, TrustBanner as TrustBannerType, useTrustStatus } from '@/api/trustLayer';
  ...
  return (
    <div className={`rounded-lg border p-4 ${bgColors[banner.type]}`}>
      <div className="flex items-start">
        <div className={`flex-shrink-0 ${iconColors[banner.type]}`}>{icons[banner.type]}</div>
...
const StatusIndicator: React.FC<{ status: string; label: string }> = ({ status, label }) => {
  ...
  return (
    ...
    >
      <span
        className={`w-2 h-2 rounded-full mr-1.5 ${dotColors[status] || 'bg-muted-foreground'}`}
        aria-hidden="true"
      />
      {label}
    </span>
