    ...
    for source in sources:
        ...
        if mask.sum() > 0:
            ...
            source_results.append(
                {"source": source, "count": mask.sum(), "accuracy": accuracy, "distribution": dist}
            )
...
def analyze_confusion_by_class(y_test, y_pred, classes):
    ...
    for i, cls in enumerate(classes):
        print(
            f"  {cls:<10} | Precision: {precision[i]:.3f} | Recall: {recall[i]:.3f} | F1: {f1[i]:.3f} | Support: {support[i]}"
        )
...
def main():
    ...
    print("=" * 60)
    print("  Model: models/signal_health_classification.joblib")
    print("  Report: reports/signal_health_report.json")
    print("=" * 60)
