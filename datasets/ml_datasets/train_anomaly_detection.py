...
def analyze_feature_importance(model, feature_names):
    ...
    importance_df = get_feature_importance(model.named_steps["classifier"], feature_names, top_n=15)
...
def main():
    ...
    results["isolation_forest"] = train_isolation_forest(X_train, X_test, y_test, numeric_cols)
    ...
    results["one_class_svm"] = train_one_class_svm(X_train, X_test, y_train, y_test, numeric_cols)
    ...
    best_model_name = max(
        results,
        key=lambda k: results[k]["metrics"].get("f1_binary", results[k]["metrics"]["f1_weighted"]),
    )
    ...
    report = {
        "model_comparison": {
            name: {
                "precision": res["metrics"].get(
                    "precision_binary", res["metrics"]["precision_weighted"]
                ),
    ...
    print("=" * 60)
    print("  Isolation Forest: models/anomaly_isolation_forest.joblib")
    print("  Supervised Model: models/anomaly_supervised.joblib")
    print("  Report: reports/anomaly_detection_report.json")
    print("=" * 60)
