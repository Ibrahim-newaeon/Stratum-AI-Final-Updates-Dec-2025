        ...
        [
            ...
            (
                ...
                RandomForestRegressor(
                    n_estimators=200, max_depth=12, min_samples_split=15, random_state=42, n_jobs=-1
                ),
            ),
        ]
    )
...
def analyze_platform_performance(df, model, X, y_roas, feature_names):
    ...
    for platform in platforms:
        ...
        if mask.sum() > 0:
            ...
            print(
                f"  {platform:<12} | Records: {mask.sum():>5} | Avg ROAS: {y_platform.mean():>6.2f}x | R²: {metrics['r2']:.3f} | RMSE: {metrics['rmse']:.3f}"
            )
...
def analyze_performance_drivers(model, feature_names, target_name):
    ...
    importance_df = get_feature_importance(model.named_steps["regressor"], feature_names, top_n=15)
...
def main():
    ...
    roas_results = train_roas_models(
        X_train, X_test, y_roas_train, y_roas_test, numeric_cols, categorical_cols
    )
    ...
    cpa_results = train_cpa_models(
        X_train, X_test, y_cpa_train, y_cpa_test, numeric_cols, categorical_cols
    )
    ...
    platform_results = analyze_platform_performance(
        df, best_roas_model, X, y_roas, numeric_cols + categorical_cols
    )
    ...
    report = {
        "roas_models": {name: res["metrics"] for name, res in roas_results.items()},
        "cpa_models": {name: res["metrics"] for name, res in cpa_results.items()},
        "best_roas_model": {"name": best_roas_name, "metrics": best_roas_metrics},
        "best_cpa_model": {"name": best_cpa_name, "metrics": best_cpa_metrics},
        "platform_analysis": platform_results,
        "roas_feature_importance": roas_importance.to_dict("records")
        if not roas_importance.empty
        else [],
        "cpa_feature_importance": cpa_importance.to_dict("records")
        if not cpa_importance.empty
        else [],
        "dataset_info": {
    ...
    print("=" * 60)
    print("  ROAS Model: models/campaign_roas_prediction.joblib")
    print("  CPA Model: models/campaign_cpa_prediction.joblib")
    print("  Report: reports/campaign_performance_report.json")
    print("=" * 60)
