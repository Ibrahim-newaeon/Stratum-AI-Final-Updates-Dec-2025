...
def train_models(X_train, X_test, y_train, y_test, numeric_cols, categorical_cols):
    ...
    ridge_pipeline = Pipeline(
        [("preprocessor", preprocessor), ("regressor", Ridge(alpha=1.0, random_state=42))]
    )
...
def cross_validate_best_model(model, X, y, cv=5):
    ...
    neg_rmse_scores = cross_val_score(
        model, X, y, cv=cv_strategy, scoring="neg_root_mean_squared_error", n_jobs=-1
    )
    ...
    return {"r2_scores": r2_scores, "rmse_scores": -neg_rmse_scores}
...
def analyze_ltv_segments(df, y_test, y_pred):
    ...
    for segment in ["Low", "Medium", "High", "Premium"]:
        ...
        if mask.sum() > 0:
            ...
            print(
                f"  {segment:<10} | Count: {mask.sum():>4} | Avg LTV: ${segment_actual.mean():>10,.2f} | MAE: ${mae:>8,.2f} | MAPE: {mape:>5.1f}%"
            )
...
def main():
    ...
    report = {
        "model_comparison": {name: res["metrics"] for name, res in results.items()},
    ...
    print("=" * 60)
    print("  Model saved: models/ltv_prediction.joblib")
    print("  Report saved: reports/ltv_prediction_report.json")
    print("=" * 60)
