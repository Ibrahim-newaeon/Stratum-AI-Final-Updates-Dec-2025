...
def generate_churn_prediction_dataset():
    ...
    for i in range(NUM_CUSTOMERS):
        ...
        days_to_renewal = np.random.randint(1, 365) if contract_type == 'annual' else np.random.randint(1, 30)
        ...
        open_tickets = np.random.choice([0, 0, 0, 1, 1, 2, 3], p=[0.5, 0.15, 0.1, 0.1, 0.08, 0.05, 0.02])
        avg_ticket_resolution_hours = np.random.exponential(24) if support_tickets_90d > 0 else 0
        nps_score = np.random.choice(range(0, 11), p=[0.02, 0.02, 0.03, 0.03, 0.05, 0.1, 0.15, 0.2, 0.2, 0.12, 0.08])
        ...
        # Billing issues
        failed_payments_90d = np.random.choice([0, 0, 0, 0, 1, 1, 2], p=[0.7, 0.1, 0.05, 0.05, 0.05, 0.03, 0.02])
        ...
        churn_date = (datetime.now() - timedelta(days=np.random.randint(1, 90))).strftime('%Y-%m-%d') if churned else None
...
def generate_ltv_prediction_dataset():
    ...
    for i in range(NUM_CUSTOMERS):
        ...
        # Acquisition cost
        channel_cac = {
            'organic': 0, 'paid_search': 150, 'paid_social': 120,
            'referral': 50, 'direct': 0, 'email': 30
        }
        ...
        ltv_12_months = base_monthly_value * 12 * engagement_factor * retention_factor * frequency_factor
...
def generate_campaign_performance_dataset():
    ...
    for i in range(NUM_CAMPAIGNS * 30):  # 30 days per campaign
        ...
        # Budget
        daily_budget = np.random.choice([50, 100, 250, 500, 1000, 2500], p=[0.2, 0.25, 0.25, 0.15, 0.1, 0.05])

        # Bid strategy
        bid_strategy = np.random.choice(['lowest_cost', 'cost_cap', 'bid_cap', 'target_cost'], p=[0.4, 0.3, 0.2, 0.1])
        ...
        base_cvr = {'conversions': 3.0, 'traffic': 0.5, 'awareness': 0.2, 'app_installs': 5.0, 'lead_gen': 8.0}
...
def generate_signal_health_dataset():
    ...
    for day in range(NUM_DAYS):
        ...
        for source in sources:
            ...
            incident_severity = np.random.choice([1, 2, 3], p=[0.6, 0.3, 0.1]) if has_incident else 0
...
def generate_anomaly_detection_dataset():
    ...
    for day in range(NUM_DAYS):
        ...
        if np.random.random() < 0.05:
            ...
            multiplier = 1 + anomaly_severity * 0.5 if anomaly_type == 'spike' else 1 - anomaly_severity * 0.3
...
def generate_data_dictionary():
    ...
    dictionary = [
        # Churn Prediction
        {'dataset': 'churn_prediction', 'field': 'customer_id', 'type': 'string', 'description': 'Unique customer identifier'},
        {'dataset': 'churn_prediction', 'field': 'account_age_days', 'type': 'int', 'description': 'Days since account creation'},
        {'dataset': 'churn_prediction', 'field': 'subscription_tier', 'type': 'categorical', 'description': 'Subscription plan: starter, professional, enterprise'},
        {'dataset': 'churn_prediction', 'field': 'mrr', 'type': 'float', 'description': 'Monthly recurring revenue'},
        {'dataset': 'churn_prediction', 'field': 'login_frequency_30d', 'type': 'int', 'description': 'Number of logins in last 30 days'},
        {'dataset': 'churn_prediction', 'field': 'login_trend_30d', 'type': 'float', 'description': 'Login frequency change rate'},
        {'dataset': 'churn_prediction', 'field': 'nps_score', 'type': 'int', 'description': 'Net Promoter Score (0-10)'},
        {'dataset': 'churn_prediction', 'field': 'churned', 'type': 'binary', 'description': 'Target: 1 if churned, 0 otherwise'},

        # LTV Prediction
        {'dataset': 'ltv_prediction', 'field': 'customer_id', 'type': 'string', 'description': 'Unique customer identifier'},
        {'dataset': 'ltv_prediction', 'field': 'rfm_score', 'type': 'int', 'description': 'Combined RFM score (3-15)'},
        {'dataset': 'ltv_prediction', 'field': 'total_revenue', 'type': 'float', 'description': 'Total historical revenue'},
        {'dataset': 'ltv_prediction', 'field': 'ltv_12_months', 'type': 'float', 'description': 'Target: Predicted 12-month LTV'},

        # Campaign Performance
        {'dataset': 'campaign_performance', 'field': 'campaign_id', 'type': 'string', 'description': 'Unique campaign identifier'},
        {'dataset': 'campaign_performance', 'field': 'platform', 'type': 'categorical', 'description': 'Ad platform: meta, google, tiktok, snapchat'},
        {'dataset': 'campaign_performance', 'field': 'objective', 'type': 'categorical', 'description': 'Campaign objective'},
        {'dataset': 'campaign_performance', 'field': 'roas', 'type': 'float', 'description': 'Target: Return on Ad Spend'},
        {'dataset': 'campaign_performance', 'field': 'cpa', 'type': 'float', 'description': 'Target: Cost per Acquisition'},

        # Signal Health
        {'dataset': 'signal_health', 'field': 'source', 'type': 'categorical', 'description': 'Data source name'},
        {'dataset': 'signal_health', 'field': 'emq_score', 'type': 'float', 'description': 'Event Match Quality score (0-10)'},
        {'dataset': 'signal_health', 'field': 'health_score', 'type': 'float', 'description': 'Overall health score (0-100)'},
        {'dataset': 'signal_health', 'field': 'health_status', 'type': 'categorical', 'description': 'Target: healthy, risk, degraded, critical'},

        # Anomaly Detection
        {'dataset': 'anomaly_detection', 'field': 'date', 'type': 'date', 'description': 'Date of observation'},
        {'dataset': 'anomaly_detection', 'field': 'spend_zscore', 'type': 'float', 'description': 'Z-score of spend metric'},
        {'dataset': 'anomaly_detection', 'field': 'is_anomaly', 'type': 'binary', 'description': 'Target: 1 if anomaly detected'},
        {'dataset': 'anomaly_detection', 'field': 'anomaly_type', 'type': 'categorical', 'description': 'Type: spike, drop, trend_break'},
    ]
